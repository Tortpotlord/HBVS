...
LEGACY = {"Jud":7,"Phi":50} # handle your Phi -> Php
...
for r in data:
    bn = str(r.get("BN","")).strip()
    b = BOOKS.get(bn) or LEGACY.get(bn)
    c = r.get("CHAPTER")
    v = r.get("VERSE")
    ...
    # Your key format: Gen1:1, Jud0:1, Phi1:1
    key = f"{bn}{c}:{v}"
    txt = " ".join([r.get(str(i),"") for i in range(1,91) if r.get(str(i),"")])
    out[key] = txt

print("Saving...")
json.dump(out, open(r"C:\HBVS\json\bible_bkchapverse.json","w",encoding="utf-8"), ensure_ascii=False)

print("DONE", len(out), "verses, skipped", bad)
print("Gen1:1:", out.get("Gen1:1",""))
print("Jud0:1:", out.get("Jud0:1",""))
print("Phi1:1:", out.get("Phi1:1",""))