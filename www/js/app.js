...
// [v7846] Load settings from localStorage
const SETTINGS = {
...
function applyGlobalSettings(){ // [v7846] Apply theme/font/size on load
...
function render5Cards(row){
  ...
  const isPreface = (currentRef.bkorder == 0 || currentRef.bkorder == 67); // [v7862] includes Epilogue
...
// [KEY FIX] Wait for Capacitor before running anything - v7.8.64
document.addEventListener('DOMContentLoaded', async () => {
  if(window.Capacitor) await Capacitor.whenReady(); // stops triggerEvent error

  applyGlobalSettings(); // [v7862] Apply theme before splash hides
  ...
  try {
    ...
    // [v7846] INJECT EPILOGUE AS DYNAMIC BOOK 67 FROM JSON
    if(SETTINGS.epilogueOn){
      ...
      if(epilogueJSON){
        ...
        verses.forEach(v=>{
          stmt.bind([v.BKORDER, v.CHAPTER, v.VERSE, v.text, v.WORDCOUNT]); // [v7864] Full text
